        ...
        <div className="p-4 sm:p-6 lg:p-8 max-w-[1200px] mx-auto">
          ...
          <div className="mb-6">
            ...
            <div className="flex items-start justify-between flex-wrap gap-4">
              <div>
                <h2 className="text-2xl font-bold text-[#1a2332]">{exam.title}</h2>
              ...
              <div className="bg-white border border-gray-200 rounded-xl px-5 py-3 shadow-sm flex flex-col min-w-[200px]">
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <span className="text-[12px] font-bold text-[#1a2332]">{completionRate}/{totalStudents}</span>
                </div>
          ...
          <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
            <div className="p-5 border-b border-gray-100 flex items-center justify-between">
              <h3 className="text-[15px] font-bold text-[#1a2332]">Student Submissions</h3>
            </div>
                ...
                <table className="w-full text-left text-[13px]">
                  <thead className="bg-gray-50 text-[10px] font-bold text-gray-400 uppercase tracking-wider">
                    <tr>
                      ...
                      <th className="px-6 py-4 border-b border-gray-100">Submitted At</th>
                      ...
                      >
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center gap-2.5">
                            ...
                            <span className="font-bold text-[#1a2332] group-hover:underline">{sub.studentName}</span>
                          </div>
                        </td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap font-bold text-[#1a2332]">
                          {sub.score !== null ? (
                            <span className="text-green-700">{sub.score} <span className="text-gray-400 font-normal text-[11px]">pts</span></span>
                          ) : (
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
      <div className="bg-white rounded-2xl w-full max-w-3xl max-h-[92vh] flex flex-col shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-gray-100 shrink-0">
          <div>
            <h2 className="text-[17px] font-bold text-[#1a2332]">Grading Submission</h2>
            <div className="flex items-center gap-2 mt-1">
              <span className="text-[12px] font-bold text-gray-500">{submission.studentName}</span>
              <span className="text-gray-300">|</span>
              <StatusBadge status={submission.status} />
            </div>
          </div>
          <button onClick={onClose} className="text-gray-400 hover:text-[#1a2332] transition-colors p-1">
        ...
        <div className="flex-1 min-h-0 overflow-y-auto px-6 py-6 bg-gray-50">
          ...
          <div className="space-y-6">
            {exam.questions.map((q, idx) => {
              ...
              const isCorrect = !isShortAnswer && studentAnswer === q.correctAnswer;

              return (
                <div key={q.id} className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm flex flex-col">
                  ...
                  {isShortAnswer ? (
                    <div className="bg-[#f8fafc] border border-gray-100 rounded-lg p-4">
                      <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-2">Student's Response</p>
                      <p className="text-[13px] text-gray-700 leading-relaxed whitespace-pre-wrap">
                        {studentAnswer || <span className="italic text-gray-400">No response provided.</span>}
                      </p>
                      ...
                      <div className="mt-4 pt-4 border-t border-gray-200 flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          <label className="text-[11px] font-bold text-gray-500">Award Points:</label>
                          <input
                            type="number"
                            min={0}
                            max={q.points}
                            value={manualScores[q.id] ?? ""}
                            onChange={(e) => setManualScores((prev) => ({ ...prev, [q.id]: Number(e.target.value) }))}
                          ...
                          />
                          <span className="text-[11px] font-bold text-gray-400">/ {q.points}</span>
                        </div>
                      </div>
                    </div>
                  ) : (
                    <div className={`rounded-lg p-4 border ${isCorrect ? "bg-green-50 border-green-100" : "bg-red-50 border-red-100"}`}>
                      <div className="flex items-center justify-between mb-2">
                        <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Student's Selection</p>
                        {isCorrect ? (
                          <span className="text-[11px] font-bold text-green-700 flex items-center gap-1"><CheckCircle2 size={13} /> Correct</span>
                        ) : (
                          <span className="text-[11px] font-bold text-red-600 flex items-center gap-1"><X size={13} /> Incorrect</span>
                        )}
                      </div>
                      <p className="text-[13px] font-semibold text-[#1a2332] mb-1">
                        {q.type === "MCQ" ? (studentAnswer ? q.options[parseInt(studentAnswer)] : "No selection") : studentAnswer || "No selection"}
                      </p>
                      {!isCorrect && (
                        <p className="text-[11px] text-gray-500 mt-2 pt-2 border-t border-red-100">
                          Correct Answer: <span className="font-bold text-[#1a2332]">{q.type === "MCQ" ? q.options[parseInt(q.correctAnswer)] : q.correctAnswer}</span>
                        </p>
                      )}
                    </div>
                  )}
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-gray-100 shrink-0 bg-white">
          <div className="text-[12px] text-gray-500 font-medium">
            Auto-graded Score: <span className="font-bold text-[#1a2332]">{submission.score}</span> / {totalMaxScore}
          </div>
          <div className="flex items-center gap-3">
            <button onClick={onClose} disabled={isPending} className="text-[12px] font-semibold text-gray-500 hover:text-[#1a2332] px-2 py-2">
              Cancel
            </button>
            <button
